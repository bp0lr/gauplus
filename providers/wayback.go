...
func (w *WaybackProvider) Fetch(domain string, results chan<- string) error {
	...
	if err != nil {
		if w.Config.Verbose {
			fmt.Printf("failed to fetch wayback pagination: %s\n", err)
		}
	...
	for page := uint(0); page < uint(pages); page++ {
		...
		if err != nil {
			if w.Config.Verbose {
				fmt.Printf("failed to fetch wayback results page %d: %s\n", page, err)
			}
		...
		if err = json.NewDecoder(resp.Body).Decode(&result); err != nil {
			_ = resp.Body.Close()
			if w.Config.Verbose {
				fmt.Printf("failed to decode wayback results for page %d: %s\n", page, err)
			}
